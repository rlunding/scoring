...
class MatchForm1Player(Form):
    ...
    score_1 = IntegerField(_('Score'), [DataRequired(), NumberRange(min=-100000, max=2147483647)], default=0)
...
class MatchForm2Players(Form):
    ...
    score_1 = IntegerField(_('Score 1'), [DataRequired(), NumberRange(min=-100000, max=2147483647)], default=0)
    team_2_id = HiddenField()
    team_2_name = StringField(_('Team 2'))
    score_2 = IntegerField(_('Score 2'), [DataRequired(), NumberRange(min=-100000, max=2147483647)], default=0)
    start_date = DateTimeField(_('Start time'), [DataRequired()], format='%Y-%m-%d %H:%M:%S')
    end_date = DateTimeField(_('End time'), [DataRequired()], format='%Y-%m-%d %H:%M:%S')
    submit = SubmitField(_('Save'))
